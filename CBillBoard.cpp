...
void CBillBoard::CalcVertex(){
	m_Vertex[0].x = m_XSize;
	m_Vertex[0].y = m_YSize;
	m_Vertex[0].z =0.0f;
	m_Vertex[0].color = m_Color;
	m_Vertex[0].tu =0.0f;
	m_Vertex[0].tv =1.0f;

	m_Vertex[1].x = m_XSize;
	m_Vertex[1].y = m_YSizeMAX;
	m_Vertex[1].z =0.0f;
	m_Vertex[1].color = m_Color;
	m_Vertex[1].tu =0.0f;
	m_Vertex[1].tv =0.0f;

	m_Vertex[2].x = m_XSizeMAX;
	m_Vertex[2].y = m_YSizeMAX;
	m_Vertex[2].z =0.0f;
	m_Vertex[2].color = m_Color;
	m_Vertex[2].tu =1.0f;
	m_Vertex[2].tv =0.0f;

	m_Vertex[3].x = m_XSizeMAX;
	m_Vertex[3].y = m_YSize;
	m_Vertex[3].z =0;
...
void CBillBoard::InitCalcVertex(int num, float x_size, float y_size){
	const float UV_SIZE = 0.1f;
...
// サイズをセット
void CBillBoard::SetSize(float x, float y,float x_max,float y_max){
	m_XSize = x;
	m_YSize = y;
	m_XSizeMAX = x_max;
	m_YSizeMAX = y_max;
...
void CBillBoard::Draw(LPDIRECT3DDEVICE9 lpDXDevice, const D3DXMATRIX& cameramat, LPDIRECT3DTEXTURE9 d3dTex){
	CalcBillBoardMatrix(cameramat);

	lpDXDevice->SetRenderState(D3DRS_LIGHTING, false);
	lpDXDevice->SetFVF(D3DFVFCUSTOM_VERTEX);
	lpDXDevice->SetTexture(0, d3dTex);
	// ワールド変換行列をセット
	lpDXDevice->SetTransform(D3DTS_WORLD, &m_mat);
	lpDXDevice->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, 2, m_Vertex, sizeof(MyVertex));
	lpDXDevice->SetRenderState(D3DRS_LIGHTING, true);
}
