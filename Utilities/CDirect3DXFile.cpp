...
//!	@note	
//==============================================================================
bool CDirect3DXFile::LoadXFile(LPCTSTR xfilename)
{
...
//!	@note	
//==============================================================================
void CDirect3DXFile::Draw()
{
	unsigned int i;
	for (i = 0; i < m_nummaterial; i++){
		m_lpd3ddevice->SetMaterial(&m_lpmeshmaterials[i]);		// マテリアルのセット
...
//!	@note	
//==============================================================================
void CDirect3DXFile::DrawWithAxis()
{
	unsigned int i;
...
void CDirect3DXFile::DrawAxis()
{
	...
	m_lpd3ddevice->SetFVF(D3DFVF_XYZ | D3DFVF_DIFFUSE);	// 頂点フォーマットをセット
	m_lpd3ddevice->SetRenderState(D3DRS_LIGHTING, false);	//   光源計算オフ（光源計算を切るとDIFFUSE色がそのままでる）

	m_lpd3ddevice->DrawPrimitiveUP(D3DPT_LINELIST, 3, &linevertices[0], sizeof(VERTEXDATA));	// ３軸を描画
	m_lpd3ddevice->SetRenderState(D3DRS_LIGHTING, true);	//   光源計算ON
}
